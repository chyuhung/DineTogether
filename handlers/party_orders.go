...
func GetPartyOrders(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := db.Query(`
			SELECT u.username, m.name, m.id, m.image_urls, m.energy_cost
			FROM orders o
		...
		for rows.Next() {
			...
			if err := rows.Scan(&order.Username, &order.MenuName, &order.MenuID, &imageURLs, &order.EnergyCost); err != nil {
